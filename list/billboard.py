...
from pyecharts.charts import Bar, EffectScatter, PictorialBar, Funnel, Pie
import pandas as pd
...
def wcsv():
    f = open('billboard.csv', mode='w', encoding='utf-8', newline='')
...
def wmysql():
    ...
    for file in files:
        if file.split('billboard.')[-1] in ['csv']:  # 判断文件是不是csv文件，file.split('.')[-1]获取‘.’后的字符串
            i += 1
...
def getdata():
    ...
    cursor = conn.cursor()
    try:
        sql_name = """ SELECT 标题 FROM list_billboard """
        cursor.execute(sql_name)
        names = cursor.fetchall()
        for name in names:
            namelist.append(name[0])
        print(namelist)
        sql_num = """ SELECT 热度 FROM list_billboard """
...
def draweBar():
    bar = Bar(
        init_opts=opts.InitOpts(width="2100px",
                                height="900px",
    ...
    bar.add_yaxis('观看人数：w', numlist, category_gap=5)
    bar.set_global_opts(
        xaxis_opts=opts.AxisOpts(
            name="排名",
            axislabel_opts=opts.LabelOpts(rotate=-45),
            axisline_opts=opts.AxisLineOpts(symbol="none", linestyle_opts=opts.LineStyleOpts(width=1)),
            axistick_opts=opts.AxisTickOpts(is_inside=True, length=20),
            axispointer_opts=opts.AxisPointerOpts(is_show=True, type_="line")
        )
    )
    bar.render('E:/PyCharm/kuaishou/templates/billboardBar.html')
...
def drawFunnel():
    ...
    funnnel.render("E:/PyCharm/kuaishou/templates/billboardfunnel.html")



def drawWordcloud():
    from PIL.Image import Image
    path_txt = 'E:\\PyCharm\\kuaishou\\billboardname.txt'
    ...
    wordcloud.to_file(os.path.join(d, "E:\\PyCharm\\kuaishou\\public\\billboardWordcloud.jpg"))
    plt.axis("off")
...
def drawEffectScatter():
   ...
   effectScatter.render("E:/PyCharm/kuaishou/templates/billboardeffectscatter.html")
...
def drawPictorialBar():
    ...
    pictorialBar.render("E:/PyCharm/kuaishou/templates/billboardpictorialbar.html")
